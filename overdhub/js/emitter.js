...
$container.on("click", ".btn-download", function () {
  ...
  playlist.tracks.forEach(function(e, i){
    var wav = audioBufferToWav(e.buffer)
    var blob = new window.Blob([ new DataView(wav) ], {
         type: 'audio/wav'
       })
    zip.file(e.name + '.wav', blob);
  });

  zip.generateAsync({type:"blob"}).then(function(content) {
    // see FileSaver.js
    downloadBlob(content, 'alltracks.zip');
  });
});
...
function displayDownloadLink(link) {
  ...
  var $link = $("<a/>", {
    'href': link,
    'download': 'mix' + dateString + '.wav',
    'text': 'Download mix ' + dateString,
    'class': 'truncate btn-download-link'
  });
  $('.btn-download-link').remove();
  $('.editor-footer').after($link);
}
...
ee.on("downloadtrack", function(track) {
  var audioBuffer = track.buffer;
  var wav = audioBufferToWav(audioBuffer)
  var blob = new window.Blob([ new DataView(wav) ], {
       type: 'audio/wav'
     })

  downloadBlob(blob, track.name);
});
